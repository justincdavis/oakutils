...
def write_autogen(f: TextIOWrapper):
    f.write(
        "# =============================================================================\n"
    )
...
def compiles_models():
    ...
    for shave in shaves:
        ...
        with open(init_final_path, "w") as f:
            ...
            for model_name in model_names:
                ...
                f.write(f"{model_name.capitalize()} : Path\n")
                f.write(f"    Path to compiled blob for {model_name.lower()} operation.\n")
            f.write('"""\n')
        ...
        outer_variable_names = var_names

    # STEP 2
    # WRITE THE bulk.py FILE
    with open(os.path.join(MODEL_FOLDER, "bulk.py"), "w") as f:
        write_copyright(f)
        write_autogen(f)
        # write the docstring
        f.write('"""\n')
        f.write("Module for getting all versions (# compiled shaves) of each operation.\n\n")
        f.write("Note\n")
        f.write("-----\n")
        f.write("This module is auto-generated\n\n")
        f.write("Attributes\n")
        ...
        # for each model we should create a tuple of strings
        # where the tuple contains the Path from each shave module
        for var_name in outer_variable_names:
            f.write(f"{var_name}: tuple[Path, ...] = (\n")
            for shave in shaves:
    ...
    with open(os.path.join(MODEL_FOLDER, "__init__.py"), "w") as f:
        ...
        f.write(f"bulk : module\n")
        f.write(f"    Contains a tuple for each model containing paths for all shaves\n")
        f.write("\n")
        f.write('"""\n')
        f.write("from __future__ import annotations\n\n")
