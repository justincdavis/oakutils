...
def check_network(func: callable):
    check_method_timout(func, func.__name__, timeout=TIME_TO_RUN + 10)  # add 5 seconds to timeout to account for setup time

def check_model(model_paths: list[str]) -> None:
    ...
    for model_path in model_paths:
        ...
        while True:
            if time.perf_counter() - t0 > TIME_TO_RUN / len(model_paths):
                break
...
def test_vpu():
    check_network(lambda: check_model([GAUSSIAN_15X15, GAUSSIAN_13X13, GAUSSIAN_11X11]))
